...
//  https://github.com/9wick/TSSqlite/blob/master/Base/TSSqlite.m
...
- (NSArray *)executeQuery:(NSString *)sql arguments:(NSArray *)args {
	...
	while ([self hasData:sqlStmt]) {
		...
		for (i = 0; i < columnCount; ++i) {
			...
			if ((columnName != nil) && (columnData != nil)) {
...
- (id)columnData:(sqlite3_stmt *)stmt columnIndex:(NSInteger)index {
	...
	if (columnType == SQLITE_INTEGER)
		return [NSString stringWithFormat:@"%d",sqlite3_column_int(stmt, index)];

	if (columnType == SQLITE_FLOAT)
		return [NSString stringWithFormat:@"%f",sqlite3_column_double(stmt, index)];
